...
\begin{aligned}
	a_k^{(n)} & = a_k(\b{x^{(n)}}) \\
	        & = \omegam{k} \\
	        & = \sum_{d = 1}^{D} w_{kd} \phi_{d}(\b{x^{(n)}}) \\
	        & = \sum_{d = 1}^{D} w_{kd} \phi_{dn} \hspace{10pt} (\phi_{d}(\b{x^{(n)}}) = \phi_{dn} とした)
\end{aligned}
$$
...
$$
\newcommand{\expA}[1]{\exp \bracket{a_{#1}}}
\newcommand{\sumExpA}[0]{\sum_{d=1}^{K} \expA{d}}
\newcommand{\fracSumExpA}[0]{\frac{\expA{k}}{\sumExpA}}
\newcommand{\partialAj}[0]{\frac{\partial}{\partial a_j}}
\begin{aligned}
	\frac{\partial P_k}{\partial a_j}
		& = \partialAj \bracket{\fracSumExpA} \\
		& = \bracket{\partialAj (\expA{k})} \frac{1}{\sumExpA} + \\
			& \expA{k} (- \frac{1}{(\sumExpA)^2}) \partialAj \bracket{\sumExpA} \\
		& = \frac{\expA{k}}{(\sumExpA)} ( \delta_{jk} - \frac{\expA{j}}{(\sumExpA)} ) \\
		& = P_k (\delta_{kj} - P_j)
\end{aligned}
$$
...
$$
\begin{align}
	\frac{\partial P_k}{\partial a_j} = P_k (\delta_{kj} - P_j)
\end{align}
$$

が成り立つため $ \eqref{eq:2.1} $ を $ \omega_{mj} $ に対して微分すると

$$
\begin{aligned}
	\frac{\partial H(\b{W})}{\partial \omega_{mj}}
		& = 
			\sum_{n=1}^{N} \sum_{k=1}^{K} \sum_{l=1}^{K}
			t_k^{(n)} \bracket{
				\frac{\partial}{\partial a_l^{(n)}} ln P_k^{(n)}
			} \frac{\partial a_l^{(n)}}{\partial \omega_{mj}} \\
		& = - \sum_{n, k, l} t_k^{(n)} P_k (\delta_{kl} - P_j) \frac{\partial}{\partial \omega_{mj}} \bracket{\sum_{d=1}^{D} \omega_{ld} \phi_{dn}} \\
...
$$
\begin{align}
	\frac{\partial H(\b{W})}{\partial \omega_{kd}} =
		\sum_{n=1}^{N} \bracket{ P_k^{(n)} - t_k^{(n)} } \phi_{dn} \label{eq:2.2}
\end{align}
$$
...
\begin{aligned}
	\omega_{kd}^{(new)} 
		& = \omega_{kd} + \eta \frac{\partial H}{\partial \omega_{kd}} \\
		& = \omega_{kd} + \eta \sum_{n=1}^{N} \bracket{ P_k^{(n)} - t_k^{(n)} } \phi_{dn} \\
	P_k^{(n)} & = \fracSumExpA \\
	\expA{k} & = \sum_{d = 1}^{D} w_{kd} \phi_{dn} \\
	\phi_{dn} & = \phi_d(\b{x^{(n)}})
\end{aligned}
$$

またこのようにして求められた最適解 $ \omega_{dk}^{\star} $ にを用いて $ \eqref{eq:0.1} $ ラベルの予測を行えることができる

### おまけ（二次の微小量）
ちなみに最急降下法ではなく二次の微小量を用いてニュートンラフソン法を使うことも可能である

$ \eqref{eq:2.2} $ をさらに $ \omega_{st} $ で微分すると

$$
\begin{aligned}
	\frac{\partial^2 H}{\partial \omega_{st} \partial \omega_{kd}} 
		& = \sum_{n=1}^{N} \frac{\partial P_k^{(n)}}{\partial \omega_{st}} \phi_{dn} \\
		& = \sum_{n=1}^{N} \sum_{l=1}^{K} \frac{\partial P_k^{(n)}}{\partial a_l^{(n)}} \frac{\partial a_l^{(n)}}{\partial \omega_{st}} \phi_{dn} \\
		& = \sum_{n=1}^{N} \sum_{l=1}^{K} P_k^{(n)} \bracket{\delta_{kl} - P_{l}^{(n)}} \delta_{sl} \phi_{tn} \phi_{dn} \\
		& = \sum_{n=1}^{N} P_k^{(n)} \bracket{\delta_{ks} - P_{s}^{(n)}} \phi_{tn} \phi_{dn} \\
\end{aligned}
$$

と得られるので、最急降下法よりさらに収束の早い方法も一応利用できる
