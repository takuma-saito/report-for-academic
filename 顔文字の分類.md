...
例えば、ID: 1 の正解値が「不満」であるが「了解」と誤分類されている。「了解」として誤識別されている理由は口の部分の「つ」に対応する部分が手の部分「(・ω･)つ」とみなされているからだろう。また、ID: 5 や ID: 7 のデータに代表されるように、学習用に用意した正解データ予測の方が正しい分類をしていると考えられるカテゴリも存在する。学習データに一部適切でないデータが混入していることも精度の低下に寄与していることがわかる。
